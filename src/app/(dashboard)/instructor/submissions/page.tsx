...
export default function InstructorSubmissions() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-100px)] space-y-4 md:space-y-6">
      ...
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-4 pt-2 md:pt-4 shrink-0 text-center md:text-left">
        <div className="space-y-1 md:space-y-1.5">
          <h1 className="text-xl md:text-3xl font-black tracking-tight text-white uppercase leading-none">Student Submissions</h1>
          <p className="text-[9px] md:text-xs text-slate-500 font-bold uppercase tracking-[0.2em]">Review work and provide feedback</p>
        </div>
        ...
        {submissions.length === 0 ? (
          ...
          <Card className="border-none bg-slate-950/20 backdrop-blur-md p-16 text-center border-dashed border border-white/10 rounded-[2rem]">
             <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-slate-500 font-medium">No submissions match your search "{searchQuery}"</p>
                <Button variant="ghost" onClick={() => setSearchQuery("")} className="text-purple-400 hover:text-purple-300 uppercase text-[10px] font-black tracking-widest">
                  Clear Search
                </Button>
             </div>
          </Card>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-3">
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                           ...
                           <div className="flex flex-col min-w-0 flex-1">
                              ...
                              <p className="text-[9px] text-slate-500 font-bold tracking-widest truncate">{sub.student?.email || "No email"}</p>
                           </div>
                        </div>
                       <div className={`px-2 py-1 rounded text-[8px] font-black uppercase tracking-widest shrink-0 whitespace-nowrap ${
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-2xl bg-slate-900 border border-white/10 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl overflow-hidden max-h-[90vh] flex flex-col"
            >
              <div className="p-5 md:p-10 space-y-6 md:space-y-8 overflow-y-auto custom-scrollbar flex-1">
                <div className="flex justify-between items-start">
                   <div className="space-y-1">
                      <div className="flex items-center gap-2 text-purple-400">
                         <GraduationCap className="h-4 w-4 md:h-5 md:w-5" />
                         <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest">Submission Review</span>
                      </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-6">
                   <a href={selectedSub?.repoUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 md:p-4 rounded-xl md:rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all cursor-pointer">
                      ...
                      <div className="text-left">
                         ...
                         <p className="text-[9px] md:text-[10px] font-bold text-white uppercase tracking-tighter truncate max-w-[120px]">View Github</p>
                      </div>
                   ...
                   {selectedSub?.liveUrl && (
                      <a href={selectedSub?.liveUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 md:p-4 rounded-xl md:rounded-2xl bg-purple-500/10 border border-purple-500/10 hover:bg-purple-500/20 transition-all cursor-pointer">
                         ...
                         <div className="text-left">
                            ...
                            <p className="text-[9px] md:text-[10px] font-bold text-purple-200 uppercase tracking-tighter truncate max-w-[120px]">Open Demo</p>
                         </div>
                ...
                {selectedSub?.comment && (
                   <div className="bg-white/5 p-4 md:p-5 rounded-xl md:rounded-2xl border border-white/5">
                      <div className="flex items-center gap-2 mb-2">
                         <MessageSquare className="h-3 w-3 md:h-3.5 md:w-3.5 text-slate-600" />
                         <span className="text-[8px] md:text-[9px] font-black text-slate-600 uppercase tracking-widest">Student's Note</span>
                      </div>
                      <p className="text-[10px] md:text-[11px] text-slate-400 font-medium italic">"{selectedSub.comment}"</p>
                   </div>
                )}

                <div className="space-y-3">
                   <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 ml-1">
                      <label className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-widest">Your Feedback & Guidance</label>
                      <button 
                   ...
                   <textarea 
                     ...
                     className="w-full bg-white/5 border border-white/10 rounded-xl md:rounded-2xl p-4 md:p-5 text-xs md:text-sm text-white focus:outline-none focus:border-purple-500/30 focus:bg-white/10 transition-all shadow-inner resize-none font-medium leading-relaxed"
                     value={feedback}
                ...
                <div className="flex flex-col gap-3 pt-2">
                   <div className="grid grid-cols-2 gap-3">
                      <Button 
                        ...
                        disabled={updating}
                        variant="outline" 
                        className="h-10 md:h-12 rounded-lg md:rounded-xl border-white/5 bg-white/5 hover:bg-yellow-500/10 hover:text-yellow-400 font-black uppercase text-[8px] md:text-[9px] tracking-widest"
                      >
                         Keep Pending
                      </Button>
                      <Button 
                        ...
                        disabled={updating}
                        variant="outline" 
                        className="h-10 md:h-12 rounded-lg md:rounded-xl border-white/5 bg-white/5 hover:bg-orange-500/10 hover:text-orange-400 font-black uppercase text-[8px] md:text-[9px] tracking-widest"
                      >
                         Improvement
                      </Button>
                   </div>
                   <Button 
                     ...
                     disabled={updating}
                     className="w-full h-10 md:h-12 rounded-lg md:rounded-xl font-black uppercase text-[8px] md:text-[9px] tracking-widest shadow-xl shadow-purple-500/20 bg-gradient-to-r from-purple-600 to-indigo-600"
                   >
                     {updating ? <Loader2 className="h-4 w-4 md:h-5 md:w-5 animate-spin" /> : (
                        <span className="flex items-center gap-2">
                           Accept Work <Send className="h-3.5 w-3.5 md:h-4 md:w-4" />
                        </span>
                     )}
                   </Button>
                </div>
